...
async function handleNewPrompt(level = 3) {
    ...
    try {
        ...
        // Formata o histórico normal da conversa
        const apiFormattedHistory = conversationHistory.messages.slice(0, -1).map(msg => ({
            role: msg.role === 'model' ? 'model' : 'user',
            parts: [{ text: msg.content }]
        }));

        // Cria a mensagem final do usuário, injetando o system prompt se for a primeira mensagem
        let finalUserPrompt = promptForApi;
        if (apiFormattedHistory.length === 0 && finalSystemPrompt) {
            finalUserPrompt = `${finalSystemPrompt}\n\n---\n\n${promptForApi}`;
        }
        apiFormattedHistory.push({ role: 'user', parts: [{ text: finalUserPrompt }] });
