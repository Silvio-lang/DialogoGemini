document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        singleBlockToggle: document.getElementById('single-block-toggle'),
        // --- ADICIONADO PARA O MODAL DO MANUAL ---
        showManualLink: document.getElementById('show-manual-link'),
        manualModalOverlay: document.getElementById('manual-modal-overlay'),
        manualModalContent: document.getElementById('manual-modal-content'),
    ...
    // Estado para a máquina de exibição
    let responseQueue = [];
    ...
    async function processNextQueueItem() {
        if (stopTypingFlag) {
            console.log("Exibição interrompida.");
            ui.unlockInput();
            return;
        }

        const isSingleBlockMode = elements.singleBlockToggle.checked;
    ...
    async function handleNewPrompt(level = 3) {
        ...
        try {
            ...
            const finalSystemPrompt = `${userSystemPrompt}\n\n[Instrução Crítica: A data e hora de hoje são EXATAMENTE ${formattedDateTime}. Suas respostas devem se basear nesta informação.]`.trim();
    ...
    async function callAnalysisAPI(instruction, analysisType) {
        if (conversationHistory.messages.length === 0) {
            alert("Não há conversa para analisar. Inicie um diálogo primeiro.");
            return;
        }
        if (isTyping) {
            alert("Aguarde a resposta atual terminar antes de iniciar uma análise.");
            return;
        }
    ...
    async function showManual() {
        const modal = elements.manualModalOverlay;
        const contentArea = elements.manualModalContent;
        if (!modal || !contentArea) return;

        modal.classList.remove('hidden');
        contentArea.innerHTML = '<div class="spinner"></div>'; // Mostra o spinner
        ...
        try {
            ...
            console.error("Falha ao carregar o manual:", error);
            contentArea.innerHTML = '<p style="color: red;">Desculpe, não foi possível carregar o manual. Por favor, tente novamente mais tarde.</p>';
        }
    }
    ...
    function performSearch() {
        ...
        chatMessages.forEach(container => {
            container.querySelectorAll('p').forEach(p => {
                p.innerHTML = p.innerHTML.replace(searchRegex, (match) => `<mark>${match}</mark>`);
            });
        });
    }
    ...
    async function runWelcomeTour() {
        const tourStepsText = [
            "👋 Olá! Este é o *DiálogoGemini!*  Aqui, a idéia é dar a você uma experiência tranquila e fluente de conversação, e também um método de registro e armazenamento organizado das conversas com a IA sobre os diversos assuntos abordados.",
            "As respostas são apresentadas gradualmente, podendo ser interrompidas a qualquer momento por você com um novo peidido, ou continuadas após a leitura de cada bloco, clicando em Continuar.",
            "Vamos lá!"," Digite seu prompt ou obtenha mais instruções, clicando abaixo em '🤓 Ajuda IA' ou na engrenagem'⚙️', acima à direita, para abrir o SideBar com as Ferramentas"
        ].join('\n\n');
    ...
    function renderSavedConversations() {
        ...
        if (savedConversations.length === 0) {
            elements.savedConversationsList.innerHTML = '<p class="empty-list-message">Nenhuma conversa salva.</p>';
            return;
        }
        ...
        input.onchange = (event) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    if (typeof importedObject !== 'object' || !Array.isArray(importedObject.messages)) { throw new Error("Arquivo de histórico de conversa inválido."); }
    ...
    function openProjectAssistant(event) { event.preventDefault(); if (window.GPT_CHAT_IFRAME_FUNCTIONS?.toogleChat) { window.GPT_CHAT_IFRAME_FUNCTIONS.toogleChat(); } else { alert('Não foi possível se comunicar com o assistente.'); } }
    ...
    function initializeApp() {
        ...
        // Se estiver local, desativa o botão do manual e muda o texto
        if (elements.showManualLink) {
            elements.showManualLink.style.pointerEvents = 'none'; // Desativa cliques
            elements.showManualLink.style.color = '#aaa'; // Deixa o link cinza
            elements.showManualLink.textContent = 'Manual (só online)';
            elements.showManualLink.title = 'Esta função requer que o app esteja hospedado (ex: GitHub Pages).';
        }
    }
        ...
        addSafeEventListener(elements.newChatLink, 'click', (e) => { e.preventDefault(); if (conversationHistory.messages.length <= 1 && !localStorage.getItem(ACTIVE_CONVERSATION_KEY)) { startNewChat(); } else { showConfirmationModal(startNewChat); } });
        addSafeEventListener(elements.showTourLink, 'click', (e) => { e.preventDefault(); if (conversationHistory.messages.length <= 1 && !localStorage.getItem(ACTIVE_CONVERSATION_KEY)) { runWelcomeTour(); } else { showConfirmationModal(runWelcomeTour); } });
        
        // --- ADICIONADO PARA O MODAL DO MANUAL ---
        addSafeEventListener(elements.showManualLink, 'click', (e) => { e.preventDefault(); showManual(); });
        addSafeEventListener(elements.manualModalCloseBtn, 'click', () => elements.manualModalOverlay.classList.add('hidden'));
