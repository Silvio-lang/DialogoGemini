...
async function handleNewPrompt(level = 3) { // Nível 3 (completo) é o padrão
    const userMessageText = elements.userInput.value.trim();
    if (!userMessageText || isTyping) return;

    // --- LÓGICA DO META-PROMPT (VERSÃO SEGURA) ---
    const metaPrompts = {
        1: "\n\n[Instrução: Responda de forma curta e direta, em no máximo 3 frases.]",
        2: "\n\n[Instrução: Responda de forma clara e com detalhes médios, em cerca de 5 a 7 linhas.]",
        3: "" // Nível 3 não precisa de instrução extra, é a resposta padrão.
    };
    
    // A mensagem original do usuário, que será salva no histórico
    const userMessage = { role: 'user', content: userMessageText, timestamp: new Date().toISOString() };
    
    // O prompt que será ENVIADO para a API (original + instrução)
    const promptForApi = userMessageText + (metaPrompts[level] || metaPrompts[3]);
    // --- FIM DA LÓGICA ---

    ui.hideContinueBtn();
    
    // Adiciona a mensagem LIMPA (sem instrução) ao histórico visível
    ...
    // Cria uma cópia temporária do histórico para a API
    const apiFormattedHistory = conversationHistory.messages.slice(0, -1).map(msg => ({ role: msg.role, parts: [{ text: msg.content }] }));
    // Adiciona a última mensagem com a instrução especial, apenas para esta chamada
    ...
    try {
        ...
        if (conversationHistory.systemPrompt) {
            requestBody.systemInstruction = { parts: [{ text: conversationHistory.systemPrompt }] };
        }
