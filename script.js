document.addEventListener('DOMContentLoaded', () => {
    ...
    function runWelcomeTour() { const tourSteps = [ "👋 Olá! Bem-vindo ao **DiálogoGemini!** Aqui, é importante que você tenha uma experiência de leitura confortável e mais humana. (clique Continuar)", "Perceba como esta resposta está sendo *apresentada gradualmente*. E você pode de **controlar essa velocidade** pelo slider **Velocidade** abaixo, deixando a escrita mais rápida ou mais lenta.", "Para respostas mais longas como esta, aparecerá o botão **Continuar**. Assim, você não será incomodado por aquela desagradável 'parede de texto' correndo para cima ⇧.", "Nesta parada é possível mudar o rumo da conversa, digitando um novo prompt, evitando a continuação desnecessária da conversa."," Agora, uma recomendação:  Para uma melhor qualidade da conversa, é conveniente **fornecer seu nome, me dar também um nome, e me designar um papel**.", "Um **primeiro prompt** assim, ajuda a produzir uma conversa muito melhor. \n\n  Veja um modelo para o primeiro prompt:\n\n\n  *'Olá! Meu nome é **Fulano** e partir de agora, seu nome será **Bia**. Aja como uma **experiente chef de cozinha**. Eu preciso de ideias para um jantar especial. Se entendeu as instruções, já me cumprimente usando seu novo nome.'*\n\n obs.: Dando a mim um nome e um papel, isto me ajudará a manter o foco.","Depois de avançar na conversa você poderá salvá-la, incluindo no título, o meu nome e o assunto. Isto ajudará a identificá-la para reativar e continuar no futuro. Assim, esta conversa poderá ir longe. Estas sugestões são favoráveis mas não obrigatórias. Você pode somente fazer sua primeira pergunta, anonimamente. Eu já estou pronto. Vamos lá?" ]; ui.lockInput(); elements.chatWindow.innerHTML = ''; (async function(){ for (let i = 0; i < tourSteps.length; i++) { const messageElement = createMessageElement('model'); await new Promise(resolve => typewriter(messageElement.querySelector('p'), tourSteps[i], resolve)); if (i < tourSteps.length - 1) { ui.showContinueBtn(); await new Promise(resolve => { const continueClickHandler = () => { elements.continueBtn.removeEventListener('click', continueClickHandler); ui.hideContinueBtn(); resolve(); }; addSafeEventListener(elements.continueBtn, 'click', continueClickHandler); }); } } localStorage.setItem('hasSeenTour', 'true'); ui.unlockInput(); })(); }
