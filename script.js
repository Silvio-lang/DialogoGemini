document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleNewPrompt(level = 3) {
        ...
        try {
            ...
            const finalSystemPrompt = `${userSystemPrompt}\n\n[Instrução Crítica: A data e hora atuais são EXATAMENTE ${formattedDateTime}. TODAS as suas respostas que envolverem datas devem se basear estritamente nesta informação.]`.trim();
           ...
           currentChunkIndex = 0;
           charsSinceLastPause = 0; // (NOVA LINHA) Reseta o contador para a nova resposta
           ui.hideLoading();
...
function displayChunks() {
    if (currentChunkIndex >= cachedChunks.length) {
        ui.unlockInput();
        return;
    }
    ...
    typewriter(messageElement.querySelector('p'), chunk, () => {
        // (LÓGICA MODIFICADA)
        charsSinceLastPause += chunk.length; // Adiciona o tamanho do chunk ao contador cumulativo
        const hasMoreChunks = currentChunkIndex < cachedChunks.length - 1;
        
        // Agora a decisão é baseada no contador CUMULATIVO
        const CHARACTER_THRESHOLD = 250; // Aumentei um pouco o limite, ajuste se necessário

        if (hasMoreChunks && charsSinceLastPause >= CHARACTER_THRESHOLD) {
            // Se atingimos o limite de caracteres E ainda há mais chunks, pausamos.
            ui.showContinueBtn();
            ui.unlockInput();
        } else if (hasMoreChunks) {
            // Se não atingimos o limite, mas ainda há chunks, continuamos automaticamente.
            currentChunkIndex++;
            displayChunks();
        } else {
            // Se não há mais chunks, apenas liberamos o input.
            ui.unlockInput();
        }
    });
}
   ...
   function handleContinue() {
    if (isTyping) return;
    ui.hideContinueBtn();
    charsSinceLastPause = 0; // (NOVA LINHA) Reseta o contador ao continuar
    currentChunkIndex++;
    displayChunks();
}
...
function loadConversation(timestamp) {
    ...
    if (conversationToLoad) {
        ...
        // Preenche o campo de nome da conversa com o nome da conversa carregada.
...
async function exportConversationToJson() {
    ...
    // Tenta usar a API moderna 'Salvar Como...'
    if (window.showSaveFilePicker) {
        try {
            ...
            // O erro 'AbortError' acontece se o usuário clica em 'Cancelar'.
    ...
    // Se a API moderna falhou ou não existe, usamos o método antigo
    // que salva diretamente na pasta Downloads.
...
async function exportConversationToMarkdown() {
    ...
    if (conversationHistory.systemPrompt) {
        markdownContent += `## Instrução do Assistente (Persona)\n\n> ${conversationHistory.systemPrompt}\n\n---\n\n`;
    }
